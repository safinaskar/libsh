...
/// C vs C++ statement
  ...
  /// Если вам нужен libsh в программе на C++, просто используйте его, но знайте, что исключения libsh несовместимы с исключениями C++. Можно поменять несколько макросов и заставить libsh бросать исключения C++. Но настоящая сиплюсплюсизация libsh означает оборачивание файловых дескрипторов в объекты, закрывающие файл в деструкторе, и тому подобные вещи. Только так можно воспользоваться всей мощью C++ для простого написания exception-safe кода
