...
/// Вам нужно указать нужный feature test macro перед #include <libsh.h> (некий минимальный для получения sigsetjmp)
/// sh_x_write возвращает ssize_t, а не size_t, хоть size_t был бы и логичнее. Я всегда возвращаю либо то же, либо void в x-функциях
/// C vs C++ statement
  /// Стандарт C и компиляторы C имеют право на существование как минимум потому, что C - это язык с context-free граматикой, а потому его компиляция идёт быстрее. И если все части GNU/Linux на C переписать на C++, то Gentoo будет собираться дольше
  ...
  /// Если вам нужен libsh в программе на C++, просто используйте его, но знайте, что исключения libsh несовместимы с исключениями C++. Можно поменять несколько макросов и заставить libsh бросать исключения C++. Но настоящая сиплюсплюсизация libsh означает оборачивание файловых дескрипторов в объекты, закрывающие файл в деструкторе, и тому подобные вещи. Только так можно воспользоваться подлинной мощью C++ для простого написания exception-safe кода
...
/// Примеры использования (во всех этих случаях shell слишком забаженный, тормознутый, не даёт низкоуровневых деталей и не мощный, а C слишком verbose)
...
///   RAII - великая вещь
///   Как минимум следующее нужно обязательно добавить в C: деструкторы и совместимые с ними исключения
